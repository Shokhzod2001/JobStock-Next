...
import JobBigCard from '../../libs/components/common/JobBigCard';
...
const AgentDetail: NextPage = ({ initialInput, initialComment, ...props }: any) => {
	...
	const {
		...
		onCompleted: (data: T) => {
			setAgentProperties(data?.getJobs?.list);
			setPropertyTotal(data?.getJobs?.metaCounter[0]?.total ?? 0);
		},
	});
				...
				<Stack className={'container'}>
					...
					<Stack className={'agent-home-list'}>
						<Stack className={'card-wrap'}>
							{agentProperties.map((job: Job) => {
								return (
									<div className={'wrap-main'} key={job?._id}>
										<JobBigCard job={job} key={job?._id} likeJobHandler={likePropertyHandler} />
									</div>
								);
							})}
						</Stack>
